...
class HomeViewModelTest {
    ...
    fun `when getStory Should Not Null and return Success`() = runTest {
        val dummyHomeResponse = DataDummy.generateDummyStoryResponse()
        val data: PagingData<ListStoryItem> = StoryPagingSource.snapshot(dummyHomeResponse.listStory)
        val expectedStoryItem = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStoryItem.value = data
        Mockito.`when`(storyRepository.getStory()).thenReturn(expectedStoryItem)

        val listHomeViewModel = HomeViewModel(storyRepository)
